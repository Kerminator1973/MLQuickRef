...
Задание №3: агрегируйте среднюю цену машин в cars. Сгруппируйте результаты по году и типу топлива на оси строк и типу коробки передач на оси столбцов.

Парвая часть задачи:

```py
cars_mean = cars.pivot_table(values="Price", index = ["Year", "Fuel"], aggfunc = "mean")
print(cars_mean.head())
```

Затем добавил группировку по оси столбцов:
...
Замечу, что в SQL такая возможность также есть, хотя на практике я её ни разу не встречал:
...
Решение из книги использует Timedelta:
...
Ключевой момент в решении из книги состоит в двух моментах: сначала мы получаем день недели, а затем вычитаем из даты аренды велосипеда день недели - это приводит к тому, что все даты выравниваются по понедельнику. Например, у нас есть шестое января 2020 года, которое является средой. Т.е. день недели среда, то dayofweek вернёт 2. Соответственно вычитая два из даты, мы получим четвертое января 2020 года, т.е. понедельник. Тонко.

Мои решения задач с четвертой по шестую:
...
Все три задачи решены правильно, но шестую можно решить более лаконично:
...
Начинаю решение с вывода списка колонок, т.к. head() выдаёт лишь ограниченную информацию.

```py
print(normalized_df.columns)
```

Фактический список колонок:

```
<class 'dict'>
Index(['season', 'episode', 'name', 'air_date', 'show', 'runtime', 'network'], dtype='str')
```

Я понял задачу следующим образом: мне необходимо выполнить фильтрацию данных и сформировать три разных DataFrame. Как будто-бы эта задача на повторение пройденного материала.

Как будто-бы решение задачи выглядит так (только для "Секретных материалов"):
