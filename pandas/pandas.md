...
Pandas умеет работать с json-файлами. При этом чаще всего осуществляется преобразование иерархии вложенных записей в плоскую последовательность, которую Pandas умеет очень эффективно обрабатывать.

JSON = _JavaScript Object Notation_. JSON относится к контейнерам данных типа "ключ/значение".
...
Следует заметить, что **json_normalize**() может не отработать корректно, если у части данных будут отсутствовать ветки в иерархии. Чтобы были нормализованы все данные, необходимо добавить отсутствующие значения. Сделать это можно с помощью трюка с установкой "значений по умолчанию":
...
Для экспорта DataFrame в JSON следует использовать метод to_json(). Например:

```py
winners.head(2).to_json(orient = "records")
```

В параметре orient можно передать аргументы: "index", "columns", "values" и "table".

Указать имя сохраняемого файла можно в первом параметре вызова:
...
По умолчанию, Pandas экспортирует данные в CSV, включая индексы, которые выделяются **индексными метками** (включая символ-разделитель "\n").

Отключить вывод индексных метод можно с помощью ключа "Index":
