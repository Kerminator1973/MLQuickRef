...
У класса DatetimeStamp есть поле dt, типа **DatetimeProperties**. Через это поле можно получать полную информацию о дате, например можно получить день месяца, или номер месяца:
...
Объект **DateOffset** позволяет выполнять операции сложения и вычитания над датами. Однако иногда простого смещения недостаточно. Pandas предоставляет специальные объекты для более сложных операций смещения (модуль "offset.py"). Пример:

```py
(disney["Date"] + pd.offset.MonthEnd()).tail()
```

В приведённом выше примере, дата и время будут округлены до конца месяца.

## Timedelta

Объект **timedelta** моделирует временные интервалы - количество времени, отделяющее два момента.

Pandas работает с интервалами посредством собственного класса Timedelta. Следует заметить, что класс **timedelta** из Python и **Timedelta** из Pandas взаимозаменяемые при использовании с операциями pandas.
...
В операции сравнения можно использовать любую детализацию продолжительности, перечисляя единицы измерения через запятую:
