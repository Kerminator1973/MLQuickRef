...
Графовые компиляторы обычно интегрированы в JAX, PyTorch и Tensor Flow.
...
Смысл мульти-индексов состоит в том, что для работы нам может потребоваться уникальный индекс строки, а отдельные ячейки не позволяют нам это сделать, т.к. значения дублируются. В этом случае имеет смысл объединять значения нескольких ячеек строки для создания общего индекса.

```py
address = ("8890 Flair Square", "Tobbside", "IL", "37206")
```

>Обобщение: индексация - типовой и естественный способ ускорить операции поиска, сортировки и фильтрации данных. Pandas позволяет создать практически любые индексы, как основываясь на таблицах, так и добавляя свои собтсвенные данные, в том числе, производные из таблиц. Предполагается, что перед началом работы с данными будут спроектированы нужные нам индексы, они будут созданы средствами Pandas, а затем все последующие операции будут выполняться очень эффективно.
...
Удалить из индекса отдельный уровень можно c использование параметров level и drop:

```py
neighborhoods.reset_index(level = "Street", drop = True)
```

Pandas позволяет установить в качестве индекса один или несколько столбцов объекта DataFrame:

```py
neighborhoods.set_index(keys = "City")
```

Для создания мультииндекса на оси строк можно указать список имён столбцов:
