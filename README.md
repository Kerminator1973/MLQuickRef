...
Считается, что экосистема PyTorch является конкурентом TensorFlow (от Google). PyTorch вырвался вперёд благодаря гибкости. Экосистема Google TPU - закрытая. Считается, что она является т.н. Vendor Lock - программные решения очень сильно сцеплены с аппаратными. Для программирования ML, у Googlt есть свой графовый компилятор XLA - Accelerated Linear Algebra.
