...
>Стемминг — это метод нормализации текста, при котором слова приводятся к базовой форме (стемме) путём отсечения аффиксов (окончаний, суффиксов, приставок).
>
>Примеры работы:
>
>- "running" → "run"
>- "happily" → "happi" (алгоритм может не всегда давать «осмысленное» слово)
>- "dogs" → "dog"
