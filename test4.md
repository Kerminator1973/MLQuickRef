...
class TextAnalyzer:
    ...
    def _get_estimation(self, word, number_of_doc, _use_idf):
        ...
        if word in feature_names:
            ...
            """
            # Извлекаем весь столбец значений для этого слова по всем документам.
            # символ двоеточия ":" это slice, т.е. эквивалент [:], в данном случае
            # означающий берём все строки. При этом используется только один
            # конкретный столбец. Как результат мы получаем вектор столбец.
            # Mетод toarray() преобразует полученную матрицу в обычный массив NumPy.
            # Метод flatten() преобразуем массив NumPy в одномерный вектор - строку.
            tf_values = tf_matrix[:, word_index].toarray().flatten()

            # Используя индекс доумента мы получаем частоту (TF), или IDF указанного 
            # слова в документе
